 ...
 */
public final class Feature1779OrdersHistoryLeak {
    ...
    public void StartLeaking() throws Exception{
        Thread thread = new LongRunningThread();
        try {
            thread.start();
            System.out.println("Running, press any key to stop.");
        ...
        @Override public void run() {
            while(running) {
                ...
                try {
                    ...
                    logger.error("Caught InterruptedException, shutting down.");
                    logger.error(ex.getMessage(), ex);
//                    System.out.println("Caught InterruptedException, shutting down.");
    ...
    static final class ChildOnlyClassLoader extends ClassLoader {
        ChildOnlyClassLoader() {
            super(Feature1779OrdersHistoryLeak.class.getClassLoader());
        }
        ...
        @Override protected Class<?> loadClass(String name, boolean resolve)
            ...
            try {
                Path path = Paths.get(Feature1779OrdersHistoryLeak.class.getProtectionDomain().getCodeSource().getLocation().getPath()
                        + "/" + LoadedInChildClassLoader.class.getName().replace(".", "/")
                        + ".class");
